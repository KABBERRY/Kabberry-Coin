...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue skkcObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        skkcObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    skkcObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("sKKCsupply", skkcObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"sKKCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 sKKC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 sKKC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 sKKC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 sKKC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 sKKC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 sKKC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 sKKC denomination\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in kabberry\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of kabberry addresses\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), sKKC_FAILED_ACCUMULATOR_INITIALIZATION);
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (sKKC excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (sKKC mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (sKKC mints included) over block range\n"
