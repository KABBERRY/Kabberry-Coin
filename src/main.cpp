...
/** Fees smaller than this (in ukkc) are considered zero fee (for relaying and mining)
...
std::map<uint256, int64_t> mapZerocoinspends; //txid, time received
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : sKKC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the sKKC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 sKKC spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend sKKC without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : sKKC spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!sKKCModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!sKKCModule::validateInput(txin, prevOut, tx, ret)){
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // be mined yet.
    if (!CheckFinalTx(tx, STANDARD_SCRIPT_VERIFY_FLAGS))
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : sKKC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-skkc");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!sKKCModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
                    ...
                    if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                        return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                                __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-skkc");
                    }
            ...
            // Check that sKKC mints (if included) are not already known
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            } else if (!hasZcSpendInputs && GetBoolArg("-relaypriority", true) && nFees < ::minRelayTxFee.GetFee(nSize) && !AllowFree(view.GetPriority(tx, chainActive.Height() + 1))) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool issKKCStake)
{
    int64_t ret = 0;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!sKKCModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
void RecalculatesKKCMinted()
{
    ...
    uiInterface.ShowProgress(_("Recalculating minted sKKC..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted sKKC..."), percent);
        }
...
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent sKKC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent sKKC..."), percent);
        }
        ...
        //Remove spends from sKKC supply
...
bool RecalculateKKCSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating KKC supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating KKC supply..."), percent);
        }
...
bool UpdatesKKCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the sKKC supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that sKKC mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateKKCSupply(Params().Zerocoin_StartHeight());
    }

    //Track sKKC money supply in the block index
    if (!UpdatesKKCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new sKKC supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 KKC.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent sKKC spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of sKKC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Kabberry Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().KabberryBadBlockTime()) && (block.nBits == (uint32_t) Params().KabberryBadBlocknBits())) {
            // accept Kabberry block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> kkcInputs;
        std::vector<CTxIn> sKKCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                sKKCInputs.push_back(stakeIn);
            }else{
                kkcInputs.push_back(stakeIn);
            }
        }
        const bool hasKKCInputs = !kkcInputs.empty();
        const bool hassKKCInputs = !sKKCInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasKKCInputs) {
                    ...
                    for (const CTxIn& kkcIn : kkcInputs)
                        if(kkcIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have sKKC inputs.
            if(hassKKCInputs) {
                for (const CTxIn& sKKCInput : sKKCInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(sKKCInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-skkc");
