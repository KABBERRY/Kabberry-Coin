...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy sKKC mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include sKKC mint output in tx %s",
        ...
        bool fCLTVIsActivated = (chainHeight >= Params().BIP65ActivationHeight());
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool issKKCStake)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 1) {
        ...
        //When sKKC is staked, masternode only gets 2 KKC
