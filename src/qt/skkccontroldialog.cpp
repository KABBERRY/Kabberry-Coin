...
std::set<std::string> sKKCControlDialog::setSelectedMints;
std::set<CMintMeta> sKKCControlDialog::setMints;

bool CsKKCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == sKKCControlDialog::COLUMN_DENOMINATION || column == sKKCControlDialog::COLUMN_VERSION || column == sKKCControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


sKKCControlDialog::sKKCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::sKKCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select sKKC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelsKKC->setProperty("cssClass", "text-main-purple");
    ui->labelsKKC_int->setProperty("cssClass", "text-main-purple");
...
sKKCControlDialog::~sKKCControlDialog()
{
    delete ui;
}

void sKKCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void sKKCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CsKKCControlWidgetItem* itemDenom(new CsKKCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CsKKCControlWidgetItem *itemMint = new CsKKCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend sKKC.";
            else if (!mint.isSeedCorrect)
                strReason = "The sKKC seed used to mint this sKKC is not the same as currently hold in the wallet";
            else
...
void sKKCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void sKKCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelsKKC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setsKKCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> sKKCControlDialog::GetSelectedMints()
{
...
void sKKCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
