...
#include "qt/kabberry/privacywidget.h"
#include "qt/kabberry/forms/ui_privacywidget.h"
#include "qt/kabberry/qtutils.h"
#include "guiutil.h"
#include "qt/kabberry/denomgenerationdialog.h"
#include "qt/kabberry/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(KabberryGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting sKKC anonymizes your KKC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new sKKC or convert back to KKC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 KKC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 sKKC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 sKKC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 sKKC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 sKKC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 sKKC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 sKKC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 sKKC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 sKKC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last sKKC Movements"));
    ...
    // Only Convert to KKC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalsKKC->setTitleClassAndText("btn-title-grey", "Total 0 sKKC");
    ui->btnTotalsKKC->setSubTitleClassAndText("text-subtitle", "Show denominations of sKKC owned.");
    ui->btnTotalsKKC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select KKC outputs to mint into sKKC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent sKKC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalsKKC, SIGNAL(clicked()), this, SLOT(onTotalsKKCClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint sKKC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of KKC to mint into sKKC"));
    }else{
        btnText = tr("Convert back to KKC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of sKKC to convert back into KKC"));
    }
...
void PrivacyWidget::onTotalsKKCClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalsKKC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalsKKC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("sKKC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_sKKC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 sKKC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("sKKC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBacksKKC(
        ...
        // Spend succeed
        inform(tr("sKKC converted back to KKC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " sKKC </b>";
    ...
    ui->btnTotalsKKC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
