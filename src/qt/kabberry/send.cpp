...
#include "qt/kabberry/send.h"
#include "qt/kabberry/forms/ui_send.h"
#include "qt/kabberry/addnewcontactdialog.h"
#include "qt/kabberry/qtutils.h"
#include "qt/kabberry/sendchangeaddressdialog.h"
#include "qt/kabberry/optionbutton.h"
#include "qt/kabberry/sendconfirmdialog.h"
#include "qt/kabberry/myaddressrow.h"
#include "qt/kabberry/guitransactionsutils.h"
...
SendWidget::SendWidget(KabberryGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("KKC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("sKKC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (KKC) or private coins (sKKC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Kabberry address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 KKC");
    ...
    setCssProperty(coinIcon, "coin-icon-kkc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onKKCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onKKCSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool issKKC = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, issKKC));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (issKKC ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendKKC = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendKKC ? AskPassphraseDialog::Context::Send_KKC : AskPassphraseDialog::Context::Send_sKKC, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendKKC) ? send(recipients) : sendsKKC(recipients)) {
...
bool SendWidget::sendsKKC(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("sKKC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from sKKC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!sKKCControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = sKKCControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendsKKC(
        ...
        inform(tr("sKKC transaction sent!"));
        sKKCControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == sKKC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 sKKC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one sKKC transaction
        ...
        Q_EMIT message("sKKC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isKKC){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any KKC to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            sKKCControlDialog *sKKCControl = new sKKCControlDialog(this);
            sKKCControl->setModel(walletModel);
            sKKCControl->exec();
            ui->btnCoinControl->setActive(!sKKCControlDialog::setSelectedMints.empty());
            sKKCControl->deleteLater();
        } else {
            inform(tr("You don't have any sKKC in your balance to select."));
        }
    }
}
...
void SendWidget::onKKCSelected(bool _isKKC){
    isKKC = _isKKC;
    setCssProperty(coinIcon, _isKKC ? "coin-icon-kkc" : "coin-icon-skkc");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress kkcAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(kkcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(kkcAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
