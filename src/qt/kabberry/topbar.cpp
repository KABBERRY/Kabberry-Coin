...
#include "qt/kabberry/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/kabberry/lockunlock.h"
#include "qt/kabberry/qtutils.h"
#include "qt/kabberry/receivedialog.h"
...
TopBar::TopBar(KabberryGUI* _mainWindow, QWidget *parent) :
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("Kabberry")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // KKC Total
    CAmount kkcAvailableBalance = balance;
    // sKKC Balance
