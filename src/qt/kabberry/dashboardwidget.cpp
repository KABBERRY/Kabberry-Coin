...
#include "qt/kabberry/dashboardwidget.h"
#include "qt/kabberry/forms/ui_dashboardwidget.h"
#include "qt/kabberry/sendconfirmdialog.h"
#include "qt/kabberry/txrow.h"
...
DashboardWidget::DashboardWidget(KabberryGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of KKC and sKKC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareKKC, "square-chart-kkc");
    setCssProperty(ui->labelSquaresKKC, "square-chart-skkc");
    setCssProperty(ui->labelKKC, "text-chart-kkc");
    setCssProperty(ui->labelsKKC, "text-chart-skkc");
    ...
    ui->labelAmountsKKC->setText("0 sKKC");
    ui->labelAmountKKC->setText("0 KKC");
    setCssProperty(ui->labelAmountKKC, "text-stake-kkc-disable");
    setCssProperty(ui->labelAmountsKKC, "text-stake-skkc-disable");
    ...
    ui->btnHowTo->setText(tr("How to get KKC or sKKC"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("KKC")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isKKC = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakesKKC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair KKC, sKKC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            kkc = (pair.first != 0) ? pair.first / 100000000 : 0;
            skkc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalKKC += pair.first;
            chartData->totalsKKC += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesKKC.append(kkc);
        chartData->valuessKKC.append(skkc);

        int max = std::max(kkc, skkc);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("KKC");
    set1 = new QBarSet("sKKC");
    ...
    set0->append(chartData->valuesKKC);
    set1->append(chartData->valuessKKC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalKKC > 0 || chartData->totalsKKC > 0) {
        setCssProperty(ui->labelAmountKKC, "text-stake-kkc");
        setCssProperty(ui->labelAmountsKKC, "text-stake-skkc");
    } else {
        setCssProperty(ui->labelAmountKKC, "text-stake-kkc-disable");
        setCssProperty(ui->labelAmountsKKC, "text-stake-skkc-disable");
    }
    forceUpdateStyle({ui->labelAmountKKC, ui->labelAmountsKKC});
    ui->labelAmountKKC->setText(GUIUtil::formatBalance(chartData->totalKKC, nDisplayUnit));
    ui->labelAmountsKKC->setText(GUIUtil::formatBalance(chartData->totalsKKC, nDisplayUnit, true));
