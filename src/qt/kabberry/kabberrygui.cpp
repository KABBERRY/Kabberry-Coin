...
#include "qt/kabberry/kabberrygui.h"
...
#include "qt/kabberry/qtutils.h"
#include "qt/kabberry/defaultdialog.h"
#include "qt/kabberry/settings/settingsfaqwidget.h"
...
const QString KabberryGUI::DEFAULT_WALLET = "~Default";

KabberryGUI::KabberryGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void KabberryGUI::createActions(const NetworkStyle* networkStyle){
 ...
 */
void KabberryGUI::connectActions() {
    ...
    connect(topBar, &TopBar::showHide, this, &KabberryGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &KabberryGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &KabberryGUI::showHide);
    ...
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &KabberryGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &KabberryGUI::execDialog);
}


void KabberryGUI::createTrayIcon(const NetworkStyle* networkStyle) {
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Kabberry Core client") + " " + networkStyle->getTitleAddText();
...
void KabberryGUI::setClientModel(ClientModel* clientModel) {
...
void KabberryGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
    ...
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &KabberryGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void KabberryGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KabberryGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void KabberryGUI::messageInfo(const QString& text){
...
void KabberryGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("Kabberry Core"); // default title
...
bool KabberryGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Kabberry Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void KabberryGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel)
        return;
...
void KabberryGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void KabberryGUI::detectShutdown() {
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
        qApp->quit();
...
void KabberryGUI::goToDashboard(){
...
void KabberryGUI::goToSend(){
    showTop(sendWidget);
}

void KabberryGUI::goToAddresses(){
...
void KabberryGUI::goToColdStaking(){
    showTop(coldStakingWidget);
}

void KabberryGUI::goToSettings(){
    showTop(settingsWidget);
}

void KabberryGUI::goToReceive(){
    showTop(receiveWidget);
}

void KabberryGUI::showTop(QWidget* view){
...
void KabberryGUI::changeTheme(bool isLightTheme){
...
void KabberryGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
void KabberryGUI::showHide(bool show){
...
int KabberryGUI::getNavWidth(){
    return this->navMenu->width();
}

void KabberryGUI::openFAQ(int section){
    showHide(true);
...
#ifdef ENABLE_WALLET
bool KabberryGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &KabberryGUI::message);
    connect(coldStakingWidget, &MasterNodesWidget::message, this, &KabberryGUI::message);
    connect(topBar, &TopBar::message, this, &KabberryGUI::message);
    connect(sendWidget, &SendWidget::message,this, &KabberryGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &KabberryGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &KabberryGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &KabberryGUI::message);
...
bool KabberryGUI::setCurrentWallet(const QString& name) {
    // Single wallet supported.
    return true;
}

void KabberryGUI::removeAllWallets() {
    // Single wallet supported.
}

void KabberryGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
...
static bool ThreadSafeMessageBox(KabberryGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KabberryGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void KabberryGUI::unsubscribeFromCoreSignals()
{
