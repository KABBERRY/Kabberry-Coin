...
CsKKCTracker::~CsKKCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CsKKCTracker::Init()
{
...
bool CsKKCTracker::Archive(CMintMeta& meta)
{
...
bool CsKKCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CsKKCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CsKKCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CsKKCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CsKKCTracker::GetSerialHashes()
{
...
CAmount CsKKCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CsKKCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CsKKCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CsKKCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CsKKCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CsKKCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CsKKCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CsKKCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CsKKCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CsKKCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CsKKCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CsKKCWallet* sKKCWallet)
{
    bool issKKCWalletInitialized = (NULL != sKKCWallet);
    CMintMeta meta;
    ...
    if (! issKKCWalletInitialized)
        sKKCWallet = new CsKKCWallet(strWalletFile);
    meta.isSeedCorrect = sKKCWallet->CheckSeed(dMint);
    if (! issKKCWalletInitialized)
        delete sKKCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CsKKCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CsKKCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CsKKCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CsKKCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CsKKCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CsKKCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CsKKCWallet* sKKCWallet = new CsKKCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, sKKCWallet);
        }
        delete sKKCWallet;
        LogPrint("zero", "%s: added %d dskkc from DB\n", __func__, listDeterministicDB.size());
    }
