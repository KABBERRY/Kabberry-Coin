...
/** Amount in KKC (Can be negative) */
typedef int64_t CAmount;

static const CAmount COIN = 100000000;
static const CAmount CENT = 1000000;

/** No amount larger than this (in KKC) is valid in a single UTXO.
 *
 * Note that this constant is *not* the total money supply, which in Kabberry
 ...
 */
static const CAmount MAX_MONEY_OUT = 60000000 * COIN;
inline bool MoneyRange(const CAmount& nValue) { return (nValue >= 0 && nValue <= MAX_MONEY_OUT); }

/**
 * Fee rate in KKC per kilobyte: CAmount / kB
 */
class CFeeRate
{
private:
