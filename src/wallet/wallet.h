...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 KKC at the same time
    ONLY_10000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for sKKC send
enum ZerocoinSpendStatus {
    sKKC_SPEND_OKAY = 0,                            // No error
    sKKC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    sKKC_WALLET_LOCKED = 2,                         // Wallet was locked
    sKKC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    sKKC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    sKKC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    sKKC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    sKKC_TRX_CREATE = 7,                            // Everything related to create the transaction
    sKKC_TRX_CHANGE = 8,                            // Everything related to transaction change
    sKKC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    sKKC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    sKKC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    sKKC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    sKKC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    sKKC_SPENT_USED_sKKC = 14,                      // Coin has already been spend
    sKKC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    sKKC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void sKKCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatesKKCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fskkcAuto) const;
    ...
    */
    ...
    CsKKCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CsKKCTracker> skkcTracker;
    ...
    ~CWallet();
    void SetNull();
    void setZWallet(CsKKCWallet* zwallet);
    CsKKCWallet* getZWallet();
    void setsKKCAutoBackups(bool fEnabled);
    ...
    /// Get 10000 KKC output and keys which can be used for the Masternode
    ...
    /** sKKC reset */
    boost::signals2::signal<void()> NotifysKKCReset;
