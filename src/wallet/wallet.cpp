...
/**
 * Fees smaller than this (in ukkc) are considered zero fee (for transaction creation)
...
{
    return skkcTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fChecksKKC = GetBoolArg("-zapwallettxes", false);
    if (fChecksKKC)
        skkcTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd skkc
            if (fChecksKKC && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use sKKC in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = skkcTracker->GetMints(true);
    ...
    return skkcTracker->GetBalance(false, false);
}
...
{
    return skkcTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = skkcTracker->ListMints(true, true, true);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 KKC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 KKC collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 KKC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 KKC.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-kabberry-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        //Masternode payment
        FillBlockPayee(txNew, nMinFee, true, stakeInput->IssKKC());
        ...
        //Mark mints as spent
        if (stakeInput->IssKKC()) {
            CsKKCStake* z = (CsKKCStake*)stakeInput.get();
...
bool CWallet::CreatesKKCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicsKKC(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreatesKKCOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new skkc output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to KKC
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), sKKC_BAD_SERIALIZATION);
        return error("%s : The transaction did not verify", __func__);
    }

    if (IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent sKKC
        receipt.SetStatus(_("The coin spend has been used"), sKKC_SPENT_USED_sKKC);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!skkcTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = skkcTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!skkcTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), sKKC_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GenerateAccumulatorWitness(&coinWitness, mapAccumulators, pindexCheckpoint)) {
            receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                              sKKC_FAILED_ACCUMULATOR_INITIALIZATION);
        ...
        if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            ...
            if (!mint.GetKeyPair(key))
                return error("%s: failed to set sKKC privkey mint version=%d", __func__, nVersion);
        ...
        try {
            ...
            if (!CheckCoinSpend(spend, accumulator, receipt)) {
                receipt.SetStatus(_("CoinSpend: failed check"), sKKC_SPEND_ERROR);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
            ...
            receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), sKKC_INVALID_WITNESS);
            return error("%s : %s", __func__, receipt.GetStatusMessage());
        }
    }
    ...
    receipt.SetStatus(_("Spend Valid"), sKKC_SPEND_OKAY); // Everything okay
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), sKKC_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), sKKC_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), sKKC_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!sKKCModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), sKKC_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = sKKC_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of sKKC"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = sKKC_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one sKKC public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the sKKC used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = skkcTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the sKKC mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), sKKC_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), sKKC_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!skkcTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            skkcTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!skkcTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            skkcTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fskkcAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fskkcAuto ? "-autoskkcbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::sKKCBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autoskkcbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autoskkcbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autoskkcbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-skkcbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-skkcbackuppath", ""));
...
)
{
    ...
    int nStatus = sKKC_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", sKKC_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = sKKC_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            skkcTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", sKKC_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", sKKC_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        skkcTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = skkcTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", sKKC_SPEND_OKAY);  // When we reach this point spending sKKC was successful
...
{
    CMintMeta meta;
    if (!skkcTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (skkcTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (skkcTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = skkcTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return skkcTracker->UpdateState(meta);
    } else {
...
void CWallet::setZWallet(CsKKCWallet* zwallet)
{
    zwalletMain = zwallet;
    skkcTracker = std::unique_ptr<CsKKCTracker>(new CsKKCTracker(strWalletFile));
}

CsKKCWallet* CWallet::getZWallet()
{
    return zwalletMain;
}

void CWallet::setsKKCAutoBackups(bool fEnabled)
{
