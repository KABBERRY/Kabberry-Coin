...
class ZerocoinSpendTest(KabberryTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, skkc_bal, kkc_bal):
            skkc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], skkc_bal)
            kkc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], kkc_bal)
            return skkc_bal, kkc_bal
        ...
        skkc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(skkc_balance, 6666)
        ...
        skkc_balance, balance = check_balances(denom_2, skkc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        skkc_balance, balance = check_balances(denom_3, skkc_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-skkc",
        ...
        _, _ = check_balances(denom_1, skkc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
